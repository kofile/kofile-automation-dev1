...
description = """
   ...
   Export search results to CSV -> Search results are downloaded in CSV format
   Open the downloaded CSVl doc and check the column data -> All columns are correctly filled with corresponding data
...
class test(TestParent):                                                                                  # noqa
    ...
    def __test__(self):
        ...
        if self.lib.PS.ps_main_page.is_search_successful():
            file_path = self.lib.CS.general.export_search_result("pdf")
            self.lib.CS.general.check_exported_file(file_path, "pdf", self.data["doc_num"])
        else:
