...
class test(ApiTestParent):
    ...
    def post_data(self):
        ...
        data["Filters"]["PartyTypes"]["$values"] = [i['Code'] for i in self.get("cs_dr_party_names") if i['Id'] != 0]
        data["PartyTypesNames"] = ['All', ]
        data["SearchFields"] = [i['FieldName'] for i in self.get("cs_dr_search_fields") if i['FieldName'] != "All"]
