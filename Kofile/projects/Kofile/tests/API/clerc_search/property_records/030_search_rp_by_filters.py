...
class test(ApiTestParent):
    ...
    def post_data(self, doc_group, party_name, search_field, use_and):
        ...
        data["Filters"]["DocumentGroups"]["$values"] = [doc_group["Id"], ]
        data["DocGroupsNames"] = [doc_group["Description"], ]
    ...
    def __test__(self):
        ...
        for doc_group, party_name, search_field in itertools.product(
                *[[i for i in self.get("cs_rp_doc_groups") if i['Id'] != 0],
                  [i for i in self.get("cs_rp_party_names") if i['Id'] != 0],
                  [i for i in self.get("cs_rp_search_fields") if i['FieldName'] != "All"]]):
            try:
                self.response = self.session.post(
                    self.get_url(self.api_urls.DocumentSearch % self.dep_id, domain_key="cs_domain"),
                    json=self.post_data(doc_group, party_name, search_field, bool(self.random.getrandbits(1))))
                assert self.response.status_code == 200, f"Actual response status: {self.response.status_code}"
                self.validate(instance=self.response.json(), schema=self.schema)
            except Exception as e:
                self.logging.error(e, exc_info=True)
                self.logging.info(f"{doc_group=}")
