...
class test(ApiTestParent):  # noqa
    ...
    def __before__(self):
        self.run_dependencies(("cs_copy_order_number",))
    ...
    def __test__(self):
        ...
        number = self.get("cs_copy_order_number")
        data = re.findall(r'(\{"OrderId[^\{]+:"%s"[^\{]+\})' % number, self.response.text)
        assert data, f"Cant find order {number} in capture queue"
        json_data = json.loads(data[0])
        self.set("copy_order_id", json_data["OrderId"])
        self.set("copy_order_status_id", json_data["OrderStatusId"])
        self.set("copy_package_id", json_data["OrderHeader_PackageId"])
