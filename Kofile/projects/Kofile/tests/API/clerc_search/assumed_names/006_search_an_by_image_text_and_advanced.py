...
class test(ApiTestParent):
    ...
    def post_data(self):
        ...
        data = self.api_test_data.cs_search_an_data.copy()
        ...
        data["Filters"]["PartyTypes"]["$values"] = [i['Code'] for i in self.get("cs_an_party_names") if i['Id'] != 0]
        data["PartyTypesNames"] = ['All', ]
        data["SearchFields"] = [i['FieldName'] for i in self.get("cs_an_search_fields") if i['FieldName'] != "All"]
    ...
    def __before__(self):
        self.run_dependencies(("cs_an_search_fields", "cs_an_party_names"))
