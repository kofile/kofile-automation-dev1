...
class test(ApiTestParent):                                                                               # noqa
    ...
    @property
    def post_data(self):
        return ('verificationTaskId', self.get("verification_task_id")),
    ...
    def __test__(self):
        ...
        self.response = self.session.get(self.get_url(
            self.api_urls.ProcessVerificationTaskAndPickupTheNextOne), params=self.post_data)
        assert self.response.status_code == 200, f"Actual response status: {self.response.status_code}"
        assert self.title in self.response.text[:200]
        self.set("order_in_archive", True)
        self.actions.step(f"Order# {self.get('order_number')} in archive")
