...
class test(ApiTestParent):                                                                          # noqa
    ...
    def post_data(self):
        return {
            'scannerTaskId': self.get("scanner_id"),
            'currentDocsCount': '0',
            'selectedFileId': '0',
        }
    ...
    def __test__(self):
        ...
        if not scan_is_finished:
            self.call_module(self.data_mapping.get("stop_scan"))
        ...
        self.set("document_id", files["DocumentId"])
        self.set("azure_file_path", files["Path"])
        self.set("scan_date", files["ScanDate"])
