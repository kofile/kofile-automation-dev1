...
class test(ApiTestParent):  # noqa
    ...
    def post_data(self):
        ...
        value = self.get('location_ids').get(self.get("last_row_data")["location"])
        assert value, f"Cant find {self.get('last_row_data')['location']} in:\n{self.get('location_ids')}"
        data["LocationId"] = value
        return data

    def __before__(self):
        self.run_dependencies(("last_row_data", "location_ids"))
    ...
    def __test__(self):
        ...
        assert row_data, f'Cant find order with id {last_row_data["order_id"]} and order number ' \
                         f'{last_row_data["order_number"]} in result by location {last_row_data["location"]}'
